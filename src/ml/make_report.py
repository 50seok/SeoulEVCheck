...
for ci,(fn,cap,cmt) in enumerate([
    ('07_pred_gu.png',
     '[그림] 예측값 vs 실제값 산점도',
     '2025년 데이터를 8:2로 분리하여, 학습에 사용하지 않은 20% 테스트셋의 정답값과 모델 예측값을 비교한 그래프다. '
     '대부분의 점이 대각선(y=x) 주변에 밀집되어 높은 일치도를 보이며, R²=0.94로 충전량 변동의 94%를 모델이 설명한다.'),
    ('08_imp_gu.png',
     '[그림] 특성 중요도 TOP12',
     'gu_금천구가 1위로, 충전소 6개에 불과한 공급 부족 구역의 극단적 패턴이 모델에 강하게 반영된 결과다. '
     '상위권이 대부분 자치구(gu_) 변수로 구성되어 위치(구)가 충전 수요의 가장 큰 결정 요인임을 확인하였다.')]):
